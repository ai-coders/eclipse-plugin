...
public class ConvDataType {
	...
	public static Float toFloat(Object in) throws Exception {
		if (in == null) {
			float douRtn = 0.0f;
			return douRtn;
	...
	public static Date toTimestamp(Object in) throws Exception {
		if (in == null || in instanceof Date) {
			return (Date) in;
		}
		Date rtn = null;
	...
	public static Object toOtherType(Object in, Class toType) throws Exception {
		...
		if (toType.equals(Integer.class) || toType.equals(int.class)) {
			...
			return toDouble(in);
		} else if (java.util.Collection.class.isAssignableFrom(toType) && in instanceof java.util.Collection) {
			return in;
		} else {
			if (in == null) {
				return null;
